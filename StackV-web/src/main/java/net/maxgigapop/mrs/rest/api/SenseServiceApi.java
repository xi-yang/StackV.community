...
public class SenseServiceApi {
    ...
    @PUT
    @Path("/{siUUID}/confirm/{qrUUID}")
    @ApiOperation(value = "Instance negotiation confirmation", notes = "Concluding and confirming neogitation and ready for provisioninig", response = Void.class, tags={ "connection",  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Successful confirmation of negotiation result", response = Void.class),
        @ApiResponse(code = 401, message = "Request unauthorized", response = Void.class),
        @ApiResponse(code = 404, message = "Resource unfound", response = Void.class),
        @ApiResponse(code = 500, message = "Server internal error", response = Void.class) })
    public Response serviceSiUUIDConfirmQrUUIDPut(@PathParam("siUUID") @ApiParam("service instance UUID") String siUUID,@PathParam("qrUUID") @ApiParam("query reference UUID") String qrUUID) {
        return Response.ok().entity("magic!").build();
    }
    ...
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Negotiate for a service instance with modified intent", notes = "Negotiate a service instance  (negotiation manditory)", response = ServiceIntentResponse.class, tags={ "computation", "connection",  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Successful service creation", response = ServiceIntentResponse.class),
        @ApiResponse(code = 400, message = "Malformed or bad request", response = Void.class),
        @ApiResponse(code = 401, message = "Request unauthorized", response = Void.class),
