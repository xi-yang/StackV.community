...
const HULL_COLOR = d3.scaleOrdinal(['#004ba0']);
...
class VisualModel {
   ...
   */
  constructor(dataModel, domElement, width = 1025, height = 768, persistence = true) {
    ...
    this.svg.root = d3.select(domElement)
    ...
    const hullDragEventHandler = new SVDragEvent({
      dragStart: function(d) { self.hullDragStart(d, d3.mouse(this)) },
  ...
  restart = () => {
    ...
    this.svg.hulls = this.svg.hulls.enter()
        ...
        .on('contextmenu', (d) => {
          d3.event.preventDefault();
          d3.event.stopPropagation();
          console.log(`(CLICK) Selected Node ${d.id}`, d);
        })
    ...
    this.svg.nodes.select('path')
        .attr('d', d => {
          return getIcon('cloud').d;
        })
        .style('transform', d => {
          return getIcon('cloud').transform;
        })
        .style('fill', d => d.fill)
        .style('display', d => {
