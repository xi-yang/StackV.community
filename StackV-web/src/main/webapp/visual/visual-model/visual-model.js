...
class VisualModel {
    ...
    */
    layoutPersistent = null;
    /**
    * Check if post render expansion is being executed
    * @type {boolean}
    */
    postRenderExpansionFinished = false;
    ...
    */
    constructor(initialServerData, domElement, width = "100%", height = "70vh", persistence = true) {
        ...
        this.dataModel = new DataModel(initialServerData);

        if (this.persistence) {
            const localLayoutData = localStorage.getItem(LP_LOCALSTORAGE_KEY);
            if (localLayoutData) {
                this.layoutPersistent = new LayoutPersistent();
    ...
    restart = () => {
        ...
        if (this.layoutPersistent && !this.postRenderExpansionFinished) {
            this.layoutPersistent.applyPostRenderExpansion(this);
            this.postRenderExpansionFinished = true;
        }
    };
    ...
    */
    tickEnd = () => {
        ...
        this.executeRenderQueueTask();
    };

    executeRenderQueueTask = () => {
    ...
    */
    canExpand = ({metadata: node}) => {
        return node && this.dataModel.canExpand(node);
    };
    ...
    * @param {boolean} force - Force expand, ignore current render state
    */
    toggleNode = (nodeWrapper, force = false) => {
        if (force || this.canRequestRender()) {
    ...
    */
    ...
    /**
    * When click svg bg, unfocus any node
    */
    ...
    */
    update(latestServerData) {
        const removeNodeIdList = this.dataModel.update(latestServerData);
        // remove selection
        removeNodeIdList.forEach(nodeId => this.highlighter.hide(nodeId));
        this.restart();
    }
}
