            ...
            {{#sriovs}}
                {{#each @root.gateways}}
                    {{#if_eq name ../hosting_gateway}}
                        {{#if_eq type 'inter_cloud_network'}} {{! probably formatted similarly to 'UCS Port Profile'}}
&lt;x-policy-annotation:data:sriov-criteria-{{@../index}}&gt;
    a            spa:PolicyData;
    spa:type     "JSON";
    spa:value    """ {
...
&lt;x-policy-annotation:action:create-aws-ops-path&gt;
    ...
    spa:exportTo &lt;x-policy-annotation:data:aws-ops-criteriaexport&gt; ; {{!TODO awsExportTo}}
&lt;x-policy-annotation:data:aws-ops-criteriaexport&gt;
    a            spa:PolicyData;
    spa:type     "JSON" ;
    spa:format   """{
